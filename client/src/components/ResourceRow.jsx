...
function rowFields(item, resource) {
  if (resource.startsWith('cr:')) {
    return [
      ...
      { value: item.age || '',    color: '#5e90b0', w: 55, right: true },
    ]
  }
  switch (resource) {
    case 'pods':
      return [
        ...
        { value: item.ready,                 color: '#72b0d0', w: 44 },
        { value: `${item.restarts}`,         color: item.restarts > 0 ? '#ffcc00' : '#5e90b0', w: 70 },
        { value: item.ip || '',              color: '#629cc0', w: 110, mono: true },
        { value: (item.node || '').replace(/^kind-/, ''), color: '#5e90b0', w: 120, mono: true },
        { value: item.age || '',             color: '#5e90b0', w: 55, right: true },
      ]
    case 'deployments':
      return [
        { value: item.status,            color: statusColor(item.status), w: 80 },
        { value: item.ready,             color: '#72b0d0', w: 50 },
        { value: `${item.upToDate ?? item.readyReplicas ?? 0}`, color: '#5e90b0', w: 72 },
        { value: `${item.available ?? 0}`,  color: '#5e90b0', w: 55 },
        { value: item.age || '',         color: '#5e90b0', w: 55, right: true },
      ]
    case 'services':
      return [
        { value: item.type,              color: '#aa55ff', w: 88 },
        { value: item.clusterIP,         color: '#629cc0', w: 100, mono: true },
        { value: item.externalIP || '',  color: item.externalIP ? '#00d4ff' : '#4e7090', w: 130, mono: true },
        { value: item.ports,             color: '#5e90b0', w: 130, mono: true },
        { value: item.age || '',         color: '#5e90b0', w: 55, right: true },
      ]
    case 'statefulsets':
      return [
        { value: item.status,    color: statusColor(item.status), w: 88 },
        { value: item.ready,     color: '#72b0d0', w: 60 },
        { value: item.age || '', color: '#5e90b0', w: 55, right: true },
      ]
    case 'daemonsets':
      return [
        { value: item.status,         color: statusColor(item.status), w: 88 },
        { value: `${item.desired}`,   color: '#629cc0', w: 72 },
        { value: `${item.ready}`,     color: item.ready < item.desired ? '#ffcc00' : '#72b0d0', w: 60 },
        { value: item.age || '',      color: '#5e90b0', w: 55, right: true },
      ]
    case 'jobs':
      return [
        { value: item.status,      color: statusColor(item.status), w: 88 },
        { value: item.completions, color: '#72b0d0', w: 100 },
        { value: item.duration,    color: '#5e90b0', w: 72 },
        { value: item.age || '',   color: '#5e90b0', w: 55, right: true },
      ]
    case 'cronjobs':
      return [
        ...
        { value: `${item.active}`,  color: item.active > 0 ? '#ffcc00' : '#52789a', w: 60 },
        { value: item.lastSchedule, color: '#5e90b0', w: 72 },
        { value: item.age || '',    color: '#5e90b0', w: 55, right: true },
      ]
    case 'ingresses':
      return [
        { value: item.hosts,     color: '#00d4ff', w: 180 },
        { value: item.address,   color: '#629cc0', w: 120, mono: true },
        { value: item.ports,     color: '#5e90b0', w: 60 },
        { value: item.age || '', color: '#5e90b0', w: 55, right: true },
      ]
    case 'configmaps':
      return [
        { value: `${item.keys} keys`, color: '#629cc0', w: 60 },
        { value: item.age,            color: '#5e90b0', w: 72, right: true },
      ]
    case 'secrets':
      return [
    ...
    case 'pvcs':
      return [
        { value: item.status,    color: statusColor(item.status), w: 88 },
        { value: item.volume,    color: '#629cc0', w: 150, mono: true },
        { value: item.capacity,  color: '#5e90b0', w: 90 },
        { value: item.age || '', color: '#5e90b0', w: 55, right: true },
      ]
    case 'pvs':
      return [
        { value: item.status,       color: statusColor(item.status), w: 88 },
        { value: item.claim,        color: '#629cc0', w: 200, mono: true },
    ...
    case 'nodes':
      return [
        ...
        { value: item.version,   color: '#5e90b0', w: 100, mono: true },
        { value: item.age || '', color: '#5e90b0', w: 55, right: true },
      ]
    case 'namespaces':
      return [
    ...
    case 'crds':
      return [
        ...
        { value: item.version,                               color: '#72b0d0', w: 80 },
        { value: item.namespaced ? 'Namespaced' : 'Cluster', color: '#5e90b0', w: 80 },
        { value: item.age || '',                             color: '#5e90b0', w: 55, right: true },
      ]
    case 'helmreleases':
      return [
        { value: item.chart,         color: '#00d4ff', w: 200 },
        { value: `v${item.version}`, color: '#629cc0', w: 80, mono: true },
        { value: item.status,        color: statusColor(item.status), w: 88 },
        { value: item.age || '',     color: '#5e90b0', w: 55, right: true },
      ]
    case 'replicasets':
    ...
    case 'hpa':
      return [
        ...
        { value: `${item.minReplicas}–${item.maxReplicas}`, color: '#629cc0', w: 80 },
        { value: `${item.currentReplicas}`,              color: '#72b0d0', w: 60 },
        { value: item.age || '',                         color: '#5e90b0', w: 55, right: true },
      ]
    case 'pdb':
      return [
        ...
        { value: `${item.allowed} allowed`,                     color: '#5e90b0', w: 90 },
        { value: item.age || '',                                color: '#5e90b0', w: 55, right: true },
      ]
    ...
    case 'serviceaccounts':
      return [
        { value: `${item.secrets} secrets`, color: '#629cc0', w: 100 },
        { value: item.age,                  color: '#5e90b0', w: 55, right: true },
      ]
    case 'resourcequotas':
      return [
        ...
        { value: `pods: ${item.pods}`,  color: '#5e90b0', w: 90, mono: true },
        { value: item.age || '',        color: '#5e90b0', w: 55, right: true },
      ]
    case 'storageclasses':
      return [
        ...
        { value: item.bindingMode,  color: '#5e90b0', w: 180 },
        { value: item.age || '',    color: '#5e90b0', w: 55, right: true },
      ]
    case 'roles':
    case 'clusterroles':
    ...
    case 'clusterrolebindings':
      return [
        ...
        { value: `${item.subjects} subject${item.subjects !== 1 ? 's' : ''}`, color: '#629cc0', w: 90 },
        { value: item.age,                                       color: '#5e90b0', w: 55, right: true },
      ]
    case 'events':
      return [
        ...
        { value: item.object, color: '#629cc0', w: 200, mono: true },
        { value: `×${item.count}`, color: item.count > 1 ? '#ffcc44' : '#5e90b0', w: 50 },
        { value: item.age || '',   color: '#5e90b0', w: 55, right: true },
      ]
    case 'containers':
      return [
        { value: item.status, color: statusColor(item.status), w: 88 },
        { value: item.pod,    color: '#629cc0', w: 200, mono: true },
      ]
    default:
      return item.status ? [
        { value: item.status, color: statusColor(item.status), w: 88 },
    ...
    >
      ...
      {nsColumnWidth > 0 && (
        <span style={{
          ...
          color: nsColor || '#5e90b0', fontFamily: 'inherit',
